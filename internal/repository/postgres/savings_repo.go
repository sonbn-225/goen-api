...
func (r *SavingsRepo) CreateSavingsInstrument(ctx context.Context, userID string, s entity.SavingsInstrument) error {
...
func (r *SavingsRepo) GetSavingsInstrument(ctx context.Context, userID string, id string) (*entity.SavingsInstrument, error) {
	pool, err := r.db.Pool(ctx)
	if err != nil {
		return nil, err
	}

	var s entity.SavingsInstrument
	...
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return nil, errors.New("savings instrument not found")
		}
		return nil, err
	}
	return &s, nil
...
func (r *SavingsRepo) ListSavingsInstruments(ctx context.Context, userID string) ([]entity.SavingsInstrument, error) {
	pool, err := r.db.Pool(ctx)
	if err != nil {
		return nil, err
	}
	...
	var out []entity.SavingsInstrument
	for rows.Next() {
		var s entity.SavingsInstrument
		if err := rows.Scan(
			&s.ID, &s.SavingsAccountID, &s.ParentAccountID, &s.Principal, &s.InterestRate, &s.TermMonths,
...
func (r *SavingsRepo) UpdateSavingsInstrument(ctx context.Context, userID string, s entity.SavingsInstrument) error {
	pool, err := r.db.Pool(ctx)
	if err != nil {
		return err
	}
...
func (r *SavingsRepo) DeleteSavingsInstrument(ctx context.Context, userID, id string) error {
	pool, err := r.db.Pool(ctx)
	if err != nil {
		return err
	}
