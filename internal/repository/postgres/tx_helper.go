...
// within a Queryer context (transaction or pool).
func CreateTransactionTx(ctx context.Context, q database.Queryer, userID uuid.UUID, tx entity.Transaction, lineItems []entity.TransactionLineItem, tagIDs []uuid.UUID) error {
	// 1. Permission Check
	switch tx.Type {
	case "expense", "income":
		if tx.AccountID == nil {
		...
		if err := requireAccountPermission(ctx, q, userID, *tx.AccountID); err != nil {
			return err
		}
	case "transfer":
		...
		if err := requireAccountPermission(ctx, q, userID, *tx.FromAccountID); err != nil {
			return err
		}
		if err := requireAccountPermission(ctx, q, userID, *tx.ToAccountID); err != nil {
			return err
		}
	}
...
func requireAccountPermission(ctx context.Context, q database.Queryer, userID, accountID uuid.UUID) error {
	var perm string
	err := q.QueryRow(ctx, "SELECT permission FROM user_accounts WHERE user_id = $1 AND account_id = $2 AND status = 'active'", userID, accountID).Scan(&perm)
	if err != nil {
...
// within a Queryer context (transaction or pool).
func DeleteTransactionTx(ctx context.Context, q database.Queryer, userID, transactionID uuid.UUID) error {
	now := utils.Now()
