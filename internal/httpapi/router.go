...
func NewRouter(cfg *config.Config, deps handlers.Deps) http.Handler {
	r := chi.NewRouter()
	...
	r.Route("/api/v1", func(r chi.Router) {
		...
		r.With(invAuth).Get("/securities/{securityId}/market-data/status", handlers.GetSecurityMarketDataStatus(deps))
		r.With(invAuth).Get("/securities/{securityId}/events", handlers.ListSecurityEvents(deps))
		r.With(invAuth).Post("/securities/{securityId}/events/refresh", handlers.RefreshSecurityEvents(deps))
		r.With(invAuth).Post("/market-data/vnstock/sync-all", handlers.RefreshMarketDataAll(deps))
		r.With(invAuth).Get("/market-data/vnstock/status", handlers.GetGlobalMarketDataStatus(deps))
		r.With(invAuth).Post("/market-data/vnstock/sync-symbol/{symbol}", handlers.RefreshMarketDataBySymbol(deps))
		r.With(invAuth).Post("/market-data/vnstock/sync-symbols", handlers.RefreshMarketDataBySymbols(deps))
